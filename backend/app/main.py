...
async def chat_endpoint(request: ChatRequest):
    ...
    """
    history = await get_session_history(request.session_id) or []
    
    # 1. Run the Multi-Agent Engine (Synchronous, can be offloaded to threadpool if needed)
    ...
    await save_session(request.session_id, history)
    
    # 3. Check if Evaluator populated the SQLite decision_matrix
    matrix_data = fetch_latest_matrix()
    if matrix_data:
        await save_decision_matrix(request.session_id, matrix_data)
